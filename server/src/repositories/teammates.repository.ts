...
export class TeammatesRepository extends Repository<UserProfile> {
	...
	async createInvitation(creatorId: string, teammateEmail: string): Promise<void> {
		...
		await this.createQueryBuilder('UserProfile').relation(UserProfile, 'pendingInvites').of(creator).add(teammate);

		await this.createQueryBuilder('UserProfile').relation(UserProfile, 'incomingInvites').of(teammate).add(creator);
	}
	...
	async changeInviteStatus(senderId: string, receiverId: string, isAcceptance: boolean) {
		...
		if (
			!senderIncomingInvites.some((user) => user.id === receiver.id) ||
		...
		if (isAcceptance) {
			await this.createQueryBuilder('UserProfile').relation(UserProfile, 'teammates').of(receiver).add(sender);

			await this.createQueryBuilder('UserProfile').relation(UserProfile, 'teammates').of(sender).add(receiver);
		}

		await this.createQueryBuilder('UserProfile')
