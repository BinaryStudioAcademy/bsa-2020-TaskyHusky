...
export function* fetchTeam(props: Props) {
	try {
		...
		NotificationManager.error('Load error', 'Error', 4000);
	}
}
...
export function* fetchLinks(props: Props) {
	try {
		...
		yield put(actions.update({ team }));
		NotificationManager.success('Data successful updated', 'Success', 4000);
	} catch (error) {
...
export function* deleteLink(props: Props) {
	try {
		...
		yield put(actions.update({ team }));
		NotificationManager.info('Link succesful deleted', 'Success', 4000);
	} catch (error) {
		NotificationManager.error('Load error', 'Error', 4000);
	}
}
