export const ua = {
	login_header: 'Вхід до аккаунту TaskyHusky',
	invalid_email: 'Введіть валідний e-mail',
	log_in: 'Увійти',
	email: 'Ел. адреса',
	password: 'Пароль',
	continue: 'Далі',
	cant_login: 'Не вдається увійти?',
	sign_up_for_an_account: 'Реєстрація',
	sign_up_header: 'Реєстрація у TaskyHusky',
	already_with_us: 'Вже з нами?',
	first_name_required: "Ім'я обов'язково",
	last_name_required: "Фамілія обов'язкова",
	invalid_password: 'Пароль має бути довжіною мінімум 6 символів',
	google_log_in: 'Увійти за допомогою Google',
	or: 'або',
	p: 'П',
	sign_up: 'Зареєструватися',
	first_name: "Ім'я",
	last_name: 'Прізвище',
	taskyhusky_logo: 'Tusky-Husky Логотип',
	people: 'Люди',
	your_work: 'Ваша праця',
	create: 'Створити',
	notifications: 'Сповіщення',
	help: 'Допомога',
	docs: 'Документація',
	community: 'Спілка',
	get_mobile: 'Отримати Tasky-Husky mobile',
	about_tasky: 'Про Tasky-Husky',
	legal: 'Правила',
	term_of_use: 'Правила використання',
	privacy_policy: 'Політика конфіденційності',
	profile: 'Профіль',
	log_out: 'Вийти',
	recent: 'Недавні',
	view_all_dashboards: 'Усі дашбоарди',
	create_dashboard: 'Створити дашбоард',
	favorite: 'Часті',
	view_all_filters: 'Усі фільтри',
	advanced_issue_search: 'Розширений пошук задач',
	view_all_projects: 'Усі проекти',
	create_project: 'Створити проект',
	search: 'Пошук...',
	projects: 'Проекти',
	filters: 'Фільтри',
	dashboards: 'Дашбоарди',
	create_issue: 'Створити задачу',
	type: 'Тип',
	priority: 'Приоритет',
	summary: 'Сумарно',
	labels: 'Мітки',
	links: 'Посилання',
	attachments: 'Файли',
	add_link: 'Додати посилання',
	add_attachment: 'Додати файл',
	description: 'Опис',
	submit: 'Відправити',
	cancel: 'Скасувати',
	tags_input_hint_1: "Натісніть 'Стрілка вгору",
	tags_input_hint_2: "' щоб застосувати зміни.",
	edit_issue: 'Оновити задачу',
	reported_by: 'Створив',
	assigned_by: 'Відповідає',
	sprint: 'Спрінт',
	loading: 'Завантаження',
	person: 'людина',
	people_rating: 'людей',
	edit: 'Редагувати',
	share: 'Поділитися...',
	open_team: 'Сторінка команди',
	add_people: 'Додати людей',
	learn_more: 'Показати більше',
	next_tip: 'Наступна порада',
	devs: 'Розробки',
	members: 'Учасники',
	member: 'учасник',
	members_lower: 'учасників',
	people_and_teams: 'Люди и команди',
	worked_on: 'Виконувалася робота',
	kanban_description:
		'Моніторинг роботи в безперервному потоці для aglie-команд ◦ Підходить командам, які контролюють обсяг роботи в backlog',
	scrum_description:
		'Керуйте історіями, завданнями і робочими процесами для scrum-команди ◦ Для команд, які виконують роботу за регулярним графіком',
	bug_tracking_description:
		'Керуйте списком завдань і помилок розробки ◦ Дуже добре підходить для команд, яким не потрібна дошка',
	enter_proj_name: "Введіть им'я проекта",
	enter_a_key: 'Введіть ключ',
	name: 'Назва',
	share_settings_with_existing_project: 'Поділитися налаштуваннями з існуючім проектом',
	template: 'Шаблон',
	change_template: 'Змінити шаблон',
	choose_classic_template: 'Обрати класичний шаблон',
	select: 'Обрати',
	whats_this: 'Що це?',
	issueKey: 'Ключ',
	create_filter: 'Створити фільтр',
	owner: 'Власник',
	stared_by: 'Позначили',
	project: 'Проект',
	group: 'Група',
	lead: 'Лідер',
	settings: 'Налаштування',
	move_to_trash: 'Перемістити в кошик',
	project_settings: 'Налаштування проекту',
	details: 'Деталі',
	example: 'Приклад',
	project_type: 'Тип проекту',
	project_category: 'Категорія проекту',
	select_image: 'обрати картинку',
	avatar: 'Аватар',
	project_lead: 'Керівник проекту',
	default_assignee: 'Призначений за замовчуванням на',
	unassigned: 'Не призначений',
	default_assignee_desc: 'Призначення за замовчуванням при створенні завдань для цього проекту.',
	project_desc: 'Опис Вашого проекту',
	save_details: 'Зберегти',
	category: 'Категорія',
	project_name: 'Введіть назву проекту',
	project_lead_name: 'Введіть им’я',
	acc_pref: 'Параметри облікового запису',
	ctrl_settings: 'Керуйте налаштуваннями облікового запису.',
	lang_reg: 'Мова та регіон',
	content_lang: 'Зміни параметрів мови відобразяться в каталозі.',
	del_acc: 'Видалення облікового запису',
	content_del:
		'Якщо видалити обліковий запис, ви втратите доступ до сервісів облікового запису Tasky-Husky, а ми назавжди видалимо ваші особисті дані. До cnjhsyrb застосовуються Політика конфіденційності та Умови обслуговування.',
	del_btn: 'Видалити обліковий запис',
	change_email: 'Змінити адресу електронної пошти',
	current_email: 'Поточна адреса електронної пошти – ',
	new_email_title: 'Нова адреса електронної пошти',
	email_placeholder: 'Укажіть нову адресу електронної пошти',
	send_email_notif: 'Відправляти мені сповіщення по електронній пошті',
	dont_send_email_notif: 'Не відправляти мені сповіщення по електронній пошті',
	save_changes: 'Зберегти зміни',
	email_notif: 'Сповіщення по електронній пошті',
	content_notif: 'Сповіщення по електронній пошті для активності задач',
	choose_option_notif: 'Оберіть опції для сповіщень по електронній пошті',
	get_email_when: 'Присилати сповіщення по електронній пошті для активності задач:',
	watching_issue: 'Ви проглядаєте задачу',
	you_reporter: 'Ви створили задачу',
	you_asignee: 'Ви відповідвальні за задачу',
	someone_mention: 'Хтось згадав вас',
	make_changes: 'Ви внесли зміни до задачі',
	back: 'Назад',
	about: 'Інформація про вас',
	placeholder_username: 'Загальнодоступне ім’я',
	placeholder_firstname: 'Им’я',
	placeholder_lastname: 'Прізвище',
	placeholder_job: 'Посада',
	placeholder_location: 'Місцезнаходження',
	placeholder_department: 'Відділ',
	placeholder_organization: 'Організація',
	security: 'Безпека',
	view_privacy_policy: 'Ознайомтесь з нашою політикою конфіденційності.',
	manage_account: 'Керуйте вашим аккаунтом',
	public_name: 'Загальнодоступне ім’я',
	job_title: 'Посада',
	department: 'Відділ',
	organization: 'Організація',
	based_in: 'Місцезнаходження',
	about_you: 'Інформація',
	content_privat_message: 'Інші користувачі бачитимуть лише ті матеріали, до яких вони мають доступ.',
	alt_experience: 'Тут ще немає проктів',
	no_work: 'Тут ще немає проктів',
	content_no_work: 'Проекти які ви створили, редагували або прокоментували за останні 30 днів.',
	work_with: 'Колеги',
	alt_user_experience: 'Немає людей для відображення',
	no_people: 'Немає людей для відображення',
	content_no_people: 'Тут будуть відображатися люди, з якими ви працювали.',
	start_team: 'Створити команду...',
	change_pass: 'Змінити пароль',
	pass_error_length: 'Пароль має бути довшим за 6 символів',
	pass_error_equal: 'Паролі мають співпадати',
	current_pass: 'Поточний пароль',
	enter_old_pass: 'Укажіть поточний пароль',
	new_pass: 'Новий пароль',
	enter_new_pass: 'Введіть новий пароль',
	repeat_pass: 'Повторіть пароль',
	placeholder_pass: 'Повторіть новий пароль',
	contact: 'Контактна інформація',
	team: 'Команди',
	save_as: 'Зберегти як',
	filter_not_created_by_you:
		'Цей фільтр був створенний іншим користувачем. Ви можете зберегти копію, але не змінюючи оригіналу.',
	save_filter: 'Зберегти Фільтр',
	filter_name: 'Назва фільтру',
	enter_filter_name: 'Введіть назву фільтру',
	delete: 'Видалити',
	close: 'Закрити',
	sure_to_delete: 'Якщо ви не впевнені, ви можете зарити це вікно.',
	permanently_delete: 'Дані проблеми видалиться на завжди, з комментаріями, прикріпленими даними.',
	delete_issue: 'Видалити проблему?',
	clear: 'Очистити',
	issues: 'Задачі',
	no_cards: 'Нема карток',
	select_icon: 'виберіть значок',
	back_to_project: 'Повернутися до проекту',
	issue_types: 'Типи завдань',
	versions: 'Версії',
	components: 'Компоненти',
	permissions: 'Дозволи',
	project_category_info: 'Ви ще не створили жодної категорії проекту.',
	project_type_info: 'Щоб змінити тип проекту, створіть новий проект і перемістіть у ньому свої задачі.',
	key_info:
		'Змінення ключа проекту призведе до повторного індексації вашого проекту та може порушити деякі зовнішні інтеграції.',
	board: 'Дошка',
	go_to_board: 'Перейти до дошки',
	assigned: 'Відповідає',
	no: 'Нема',
	searchIssue: 'Пошук',
	containText: 'Включає текст',
	reporter: 'Репортер',
	status: 'Статус',
	work_places: 'Проекти',
	my_profile: 'Мій профіль',
	view_all: 'показати більше',
	hide: 'сховати',
	text: 'Текст',
	enter_comment_text: 'Введіть текст вашего коментаря...',
	comments: 'Коментарі',
	post_comment: 'Відправити коментар',
	boards: 'Дошки',
	create_board: 'Створити дошку',
	view_all_boards: 'Усі дошки',
	advanced_search: 'Розширений пошук задач',
	create_sprint: 'Створити спринт',
	no_sprints_header: 'Активні спринти відсутні',
	no_sprints_header_subheader: 'Плануйте роботу своєї команди. Створіть свій перший спринт.',
	no_issues_in_sprint: 'Сплануйте спринт, створивши декілька завдань.',
	no_issues_in_backlog: 'Беклог - це список задач вашої команди',
	backlog: 'Беклог',
	edit_sprint: 'Редагувати спринт',
	delete_sprint: 'Видалити спринт',
	sprint_cannot_be_deleted: 'Неможливо видалити спринт',
	delete_sprint_with_no_issues_warning: 'Ви впевнені, що хочете видалити цей спринт? Цю дію не можна скасувати.',
	delete_sprint_with_issues_warning:
		'Ви впевнені, що хочете видалити цей спринт? Всі поточні завдання будуть перенесені в Беклог. Цю дію не можна скасувати.',
	enter_sprint_name: 'Введіть назву спринту',
	mark_sprint_as_active: 'Спринт буде позначено як активний',
	mark_sprint_as_inactive: 'Спринт буде позначено як неактивний',
	mark_sprint_as_completed: 'Спринт буде позначено як завершений',
	mark_sprint_as_not_completed: 'Спринт буде позначено як незавершений',
	sprint_name: 'Назва спринту',
	sprint_name_cannot_be_empty: 'Назва спринту не може бути порожнім',
	my_work: 'Моя робота',
	creator: 'Творець',
	no_projects: 'Не знайдено проектів',
	createAt: 'Створений',
	updatedAt: 'Оновлений',
	save_not_secure: 'Збереження недостатньо надйного паролю',
	save_not_secure_text:
		'Хочете зберегти ненадійний пароль? Для того щоб зробити пароль надыйним ви можете додати символи, цифри та букви верхнього регістру.',
	warning: 'Предупреждение',
	issue_was_deleted_message_part_2:
		'була видалена. Ця сторінка стане недоступною, як тільки ви перезавантажите або закриєте її.',
	issue: 'Задача',
	key: 'Ключ',
	sign_up_with_google: 'Продовжити з допомогою Google',
	anonymous: 'Аноним',
	watchers: 'Відстежують',
	watch: 'Відстежувати',
	unwatch: 'Не відстежувати',
	see_it: 'Дивитися!',
	mark_notif_as_read: 'Відмітити як прочитане',
	show_notifs_during_last_10_days: "Показати всі об'яви за останні 10 днів",
	mark_notif_as_unread: 'Відмітити як непрочитане',
	mark_all_as_read: 'Відмітити все як прочитане',
	watching: 'Відстежуєтся',
	not_watching: 'Не відстежуєтся',
	add: 'Додати',
	search_name_email: 'Пошук по імені або email...',
	send_confirm_link: 'Відправити посилання для підтвердження e-mail',
	confirm_email_change: 'Підтвердіть змінення email',
	type_pass_confirm_email: 'Введіть пароль для підтвердження зміни email',
	email_update: 'Email змінено',
	email_confirm_pass: 'Введіть свій email для підтвердження відправки посилання на зміну паролю',
	forgot_pass: 'Забули пароль?',
	and: 'та',
	drag_n_drop_files_here: 'Перетягніть файли сюди',
	click_to_browse: 'Натисніть для завантаження',
	files_are_available: 'файли доступні',
	click_to_download: 'Натисніть для скачування',
	startDate: 'Початок',
	endDate: 'Кінець',
	duration: 'Тривалість',
	dateValidationError: 'Початок спринта повен бути раніше, ніж кінець.',
	week: 'тиждень',
	custom: 'керований',
	no_results_found: 'Нічого не знайдено',
	add_teammates: 'Додати користувачів в команду',
	no_more_4_people_can_ba_added: 'Ви можете додати одночасно до 4 учасників',
	accept: 'Підтвердити',
	you_are_going_to_delete_team: 'Ви збираєтеся видалити дану команду',
	deleting_will_destroy_information:
		"Видалення команди знищить всю інформацію, пов’язану з нею. Ім'я команди, учасники, опис, посилання на залучені матеріали будуть видалені.",
	deleting_team_cannot_be_prevented: 'Видаленню команди не можна буде запобігти. Ви впевнені, що хочете продовжити?',
	Im_sure: 'Я підтверджую',
	add_some_description: 'Добавте короткий опис команди',
	edit_fields: 'Редагувати поля',
	delete_team: 'Видалити',
	web_address: 'Web адреса',
	title: 'Заголовок',
	for_example_my_first_project: 'Наприклад: Посилання на демо проекту',
	add_small_specification: 'Додайте невелике пояснення, щоб інші члени вашої команди дізналися, чому це важливо',
	small_description: 'Короткий опис',
	delete_link: 'Видалити посилання на матеріал',
	link_to: 'Посилання на',
	will_be_deleted_from_section: 'буде видалено з розділу посилань на матеріали команди',
	project_key: 'Ідентифікатор проекту',
	add_label: 'Додати мітку',
	label_text: 'Текст мітки',
	label_pick_color: 'Підберіть колір',
	result: 'Результат:',
	no_label: 'Ярликів немає',
	delete_label: 'Видалити мітку',
	delete_label_confirm: 'Ви дійсно хочете видалити цю мітку?',
	team_owner: 'Власник команди',
	you_are_the_first: 'Власник команди перший і поки що єдиний учасник команди',
	you_are_going_to_remove_user: 'Ви плануєте видалити користувача',
	from_team: 'з команди',
	once_it_happend_he_will_lose_access_to_teamwork:
		'Коли ви це зробите, даний користувач втратить доступ до матеріалів та проектів команди',
	but_you_can_still_add_him_in_future:
		'Але ви зможете долучити його знову до команди у майбутньому, якщо це необхідно',
	error_load_data: 'Помилка завантаження даних',
	data_updated_succesful: 'Інформація успішно оновлена',
	error_load_team_links: 'Помилка завантаження посилань команди',
	link_has_been_deleted: 'Посилання на матеріал успішно видалено',
	error_update_data: 'Помилка оновлення даних',
	error_add_users_to_team: 'Помилка додавання користувачыв в команду',
	team_has_been_deleted: 'Команда успішно видалена',
	error_delete_team: 'Помилка видалення команди',
	user_has_been_removed: 'Користувач успішно видалений',
	error_delete_user: 'Помилка видалення користувача',
	success: 'Успіх',
	error: 'Помилка',
	info: 'Інформація',
	search_people_page_placeholder: 'Пошук користувачів та команд',
	create_team: 'Створити команду',
	add_people_text: 'Не вистачає когось? Надрукуйте його електронну пошту нижче.',
	add_people_email_placeholder: 'Введіть електронну адресу',
	send: 'Надіслати',
	create_team_modal_header: 'Створити нову команду',
	create_team_modal_text:
		'Створіть можливість для своїх колег працювати в одному місці, додавши їх до команди. Залишайтеся на зв’язку використовуючи @згадки, працюйте над спільною роботою та ефективно керуйте усім на сторінці профілю команди.',
	team_name: 'Назва команди',
	create_team_modal_placeholder: 'Як ви назвете свою команду?',
	scrum: 'SCRUM',
	kanban: 'Канбан',
	create_a_board: 'Створити дошку',
	scrum_focuses_on_planning:
		'Scrum фокусується на плануванні, разпределенні і виконанні обмежених за часом фрагментів роботи, які називаються Спринтами.',
	kanban_focuses_on_planning:
		'Канбан зосереджується на візуалізації робочого процесу та обмеженні незавершеного виробництва для полегшення поступові вдосконалення вашого існуючого проекту',
	create_scrum_board: 'Створити SCRUM дошку',
	create_kanban_board: 'Створити Канбан дошку',
	board_created_with_new_software_project: 'Дошка створена за допомогою нового програмного проекту',
	board_from_an_existing_saved_filter: 'Дошка із існуючого збереженого фільтру',
	board_from_an_existing_project: 'Дошка із існуючого проекту',
	board_name: 'Назва дошки',
	create_board_final_text: 'Виберіть один або декілька проектів, щоб додати їх до дошки',
	name_this_board: 'Назвіть дошку',
	next: 'Далі',
	all_boards: 'Всі дошки',
	admin: 'Адмін',
	edit_settings: 'Редагувати',
	move: 'Перемістити',
	view_issue: 'Переглянути проблему',
	criteria: 'Критерій',
	more: 'Більше',
	update: 'Оновити',
	export: 'Експорт',
	find_by: 'Знайти по колонці',
	Projects: 'Проекти',
	Status: 'Статус',
	Type: 'Тип',
	Assignee: 'Відповідальний',
	Creator: 'Власник',
	Priority: 'Пріоритет',
	Description: 'Опис',
	Comment: 'Коментар',
	Summary: 'Сумарно',
	of: 'з',
	createdAt: 'Створений',
	P: 'П',
	enter_project_name: 'Напишіть назву проекту',
	enter_your_key: 'Напишіть власний ключ проекту',
	enter_your_project_URL: 'Напишіть GitHub URL проекту',
	min4_max40_length_message: 'Має містити принаймні 4 і максимум 40 символів.',
	delete_user_qstn: 'Ви впевнені, що хочете видалити свій аккаунт?',
	delete_user_text: "Видалення цього аккаунту призведе до втрати всіх даних зв'язаних з цим аккаунтом",
	delete_btn: 'Видалити',
	my_open_issues: 'Мої пототчні задачі',
	my_done_issues: 'Мої віконані задачі',
	user_was_updated: 'Данні користувача змінено',
	could_not_update_user: 'Данні користувача не змінено',
	pass_was_updated: 'Пароль змінено',
	could_not_update_pass: 'Пароль не змінено',
	email_was_updated: 'Email изменено',
	could_not_update_email: 'Email не змінено',
	cold_not_get_user: 'Коричтувач не знайдений',
	avatar_was_updated: 'Аватар профілю змінено',
	could_not_update_avatar: 'Аватар профілю не змінено',
	could_not_send_email: 'Лист не відправлено',
	email_was_send: 'Лист відправлений',
	choose_kanban: 'Виберіть Kanban, якщо хочете',
	choose_kanban_item_1: 'підвищити продуктивність та ефективність',
	choose_kanban_item_2: 'мати гнучкість у розробці',
	choose_kanban_item_3: 'обмежити кількість неазкінченої роботи',
	choose_kanban_item_4: 'уникати багатозадачності',
	choose_scrum: 'Виберіть Scrum, якщо хочете',
	choose_scrum_item_1: 'забезпечити гнучкість при зміні вимог',
	choose_scrum_item_2: 'оцінювати обсяг роботи, яку потрібно виконати',
	choose_scrum_item_3: 'будувати продукт крок за кроком',
	choose_scrum_item_4: 'заощадити час і гроші',
	more_about: 'Докладніше про',
	story_point: 'Оцінка складності',
	user_does_not_exist: 'Такого користувача не існує',
	sign_in: 'Увійти',
	create_an_account: 'Створити обліковий запис',
	already_have_a_account: 'Ви вже маєте обліковий запис?',
	label_text_validation: 'Назва мітки має бути унікальною та не повинна бути порожньою.',
	enter_label_text: 'Введіть текст мітки',
	create_column: 'Створити колонку',
	is_resolution_set: 'Is resolution set',
	todo: 'До виконання',
	in_progress: 'Виконується',
	done: 'Виконано',
	the_comments_text_is_copied_to_clipboard: 'Tекст цього коментаря збережений у буфер обміну.',
	filter_was_saved: 'Filter was saved',
	filter_was_updated: 'Filter was updated',
	could_not_save_filter: "Can't save filter",
	could_not_update_filter: "Can't update filter",
	yes: 'Так',
	no_filters: 'Фільтрів не знайдено',
	add_changes: 'Додайте зміни, щоб розблокувати кнопку',
	are_you_sure: 'Ви впевнені?',
	are_you_sure_delete_column: 'Ви впевнені, що хочете видалити цю колонку?',
	go_to_columns_settings: 'Перейти до налаштувань колонок',
	remove_user_from_project: 'Видалити з проекту',
	team_header: 'Команда',
	my_team: 'Моя команда',
	active_sprint: 'Активний спринт',
	reports: 'Звіти',
	click_here_to_add_link: 'Натисніть, щоб додати посилання...',
	click_here_to_add_attachment: 'Натисніть, щоб додати файл...',
	no_more_labels: 'Більше нема міток. TIP: Щоб отримати доступ до міток, треба обрати проект',
	column: 'Колонка',
	you_current_location: 'Ваше поточне місцерозташування ',
	you_have_not_specify_location: 'Вы ще не вказали ваше місцерозташування',
	choose_your_location: 'Виберіть ваше місцерозташування',
	apply_current_location: 'Знайти поточне місцерозташування',
	apply_this_location: 'Вибрати це місцерозташування',
	no_notifications: 'У вас немає нових сповіщень',
	landing_title: 'Кращий інструмент розробки для agile-команд',
	landing_intro_title: 'Кращі команди розробників випускають ПО швидко і часто.',
	landing_intro_description:
		'Рішення TaskyHusky розроблено таким чином, щоб кожен член Вашої команди розробників міг планувати, відстежувати і випускати чудове ПО.',
	landing_block_title_planning: 'Гнучке планування',
	landing_block_description_planning:
		'Scrum? Є. Kanban? Є. TaskyHusky надає безліч варіантів планування, завдяки чому ваша команда зможе створювати плани оптимальним способом.',
	landing_block_title_estimations: 'Точні оцінки',
	landing_block_description_estimations:
		'Завдяки оцінками ваша команда буде працювати якісніше і ефективніше. Ви можете використовувати очки за історії, годинник, розміри футболок або власні методики оцінки - TaskyHusky все це підтримує.',
	landing_block_description_prioritization: 'Розстановка пріоритетів на основі цінності',
	landing_block_title_prioritization:
		'Впорядкувати призначені для користувача історії, завдання і баги в беклоге продукту можна простим перетягуванням. Тепер все найважливіше для клієнта завжди буде нагорі.',
	landing_block_description_execution: 'Прозорість виконання',
	landing_block_title_execution:
		'З TaskyHusky робота вашої команди стане прозорішою і всі її члени завжди будуть в курсі того, що відбувається, де б вони не знаходилися - в сусідніх кімнатах або на різних континентах.',
	you_do_not_have_projects: 'Ви ще не створювали проєктів',
	you_do_not_have_issues: 'Ви не відповідальні ні за одну задачу',
	you_do_not_have_activity: 'У вас ще немає історії активності',
};
